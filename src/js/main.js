(function($) {
  $(document).ready(function() {
    ...
    function drawTitle() {
      for (var i = canvases.length - 1; i >= 0; i--) {
        if (canvases[i].type && canvases[i].type == "icon") {} else {
          var ctx = canvases[i].ctx;
          if (title && title.length != 0) {
            ctx.font = 'bold ' + canvases[i].title.size + "px 'Noto Sans JP'";
            ctx.fillStyle = 'rgb(256, 256, 256)';
    ...
    function drawSubTitle() {
      ...
      for (var i = canvases.length - 1; i >= 0; i--) {
        if (canvases[i].type && canvases[i].type == "icon") {} else {
          var ctx = canvases[i].ctx;
          if (subTitle && subTitle.length != 0) {
            ctx.font = canvases[i].subTitle.size + "px 'Noto Sans JP'";
            ctx.fillStyle = 'rgb(256, 256, 256)';
            ctx.textAlign = "center";
    ...
    if (staticIcons) {
      for (var i = 0; i < staticIcons.length; i++) {
        //console.log(staticIcons[i]);
        var si ='<div class="row" style="margin-bottom:20px;"><div class="col">'
        + '<h4><img src="data:image/png;base64,' + staticIcons[i].base64 + '" /> ' + staticIcons[i].label + '</h4>'
        + '<div class="form-group"><textarea id="base64-static-' + i + '" class="form-control base64" readonly="" rows="12" cols="80">' + splitByLength(staticIcons[i].base64, 76).join('\n') + '</textarea></div>'
        + '</div></div>';
        $('#staticIcons').append(si);
      }
    }
